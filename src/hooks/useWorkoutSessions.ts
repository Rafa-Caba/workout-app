    ...
    return useMutation<unknown, ApiError, CreateSessionBody>({
        mutationFn: async (payload) => {
            ...
            if (!sessionId) {
                ...
                // The UI can show the ApiError toast (or you can catch and toast at call-site).
                throw {
                    status: 500,
...
export function usePatchWorkoutSession(args: {
    ...
    return useMutation<unknown, ApiError, PatchSessionBody>({
        mutationFn: (payload) => patchSession(args.date, args.sessionId, payload, { returnMode: args.returnMode ?? "day" }),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ["workoutDay", args.date] });
            if (args.weekKey) qc.invalidateQueries({ queryKey: ["planVsActual", args.weekKey] });
        },
...
export function useDeleteWorkoutSession(args: {
    ...
    return useMutation<unknown, ApiError, { deleteMedia?: boolean } | undefined>({
        mutationFn: (payload) =>
            deleteSession(args.date, args.sessionId, { returnMode: args.returnMode ?? "day", deleteMedia: payload?.deleteMedia }),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ["workoutDay", args.date] });
            if (args.weekKey) qc.invalidateQueries({ queryKey: ["planVsActual", args.weekKey] });
        },
