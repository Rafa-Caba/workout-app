...
export function extractWeekKpis(weekSummary: unknown): { kpis: WeekKpis; bySessionType: WeekBySessionTypeRow[] } {
    ...
    const training = isRecord(weekSummary.training) ? weekSummary.training : null;
    const sleep = isRecord(weekSummary.sleep) ? weekSummary.sleep : null;

    const daysCount = numOrDash(weekSummary.daysCount);
    ...
    const bySessionType: WeekBySessionTypeRow[] = bySessionTypeRaw.map((row) => {
        ...
        const mediaCountPerType =
            typeof maybeMedia === "number" && Number.isFinite(maybeMedia) ? maybeMedia : undefined;
