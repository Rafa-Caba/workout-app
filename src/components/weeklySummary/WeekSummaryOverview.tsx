...
function findMostActiveDay(
    ...
    return rows.reduce((best, current) =>
        compareActivity(current, best) > 0
            ? current
            : best,
    );
}
...
function countDaysWithRecords(
    days: readonly CalendarDayFull[],
): number {
    return days.filter((day) => {
        const hasSleep = Boolean(
            day.sleep || day.sleepSummary,
        );

        const sessionsCount = Array.isArray(
