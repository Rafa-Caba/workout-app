...
// Admin users orchestration with MUI list/form/purge components.
...
function getErrorMessage(error: unknown, lang: string): string {
    ...
    const fallback = lang === "es" ? "Ocurrió un error inesperado." : "An unexpected error occurred.";
...
function buildSubmitError(form: UserFormValues, isEditing: boolean, lang: string): string | null {
    if (form.name.trim().length === 0) return lang === "es" ? "Nombre requerido." : "Name is required.";
    if (form.email.trim().length === 0) return lang === "es" ? "Email requerido." : "Email is required.";
    if (!isEditing && form.password.trim().length === 0) return lang === "es" ? "Contraseña requerida." : "Password is required.";
    if (form.coachMode === "TRAINEE" && !form.assignedTrainer) {
        return lang === "es" ? "Si el usuario es TRAINEE, debes asignar un trainer." : "If user is TRAINEE, you must assign a trainer.";
    }
    return null;
}
...
function buildMediaItem(user: AdminUser, lang: string): MediaFeedItem | null {
    ...
    return {
        ...
        resourceType: "image",
        format: null,
        createdAt: user.lastLoginAt ?? readCreatedAt(user) ?? new Date().toISOString(),
...
export function AdminUsersSection() {
    ...
    React.useEffect(() => {
        void loadUsers();
    }, [activeFilter, loadUsers, page, roleFilter, search]);
    ...
    React.useEffect(() => {
        ...
        async function loadTrainers() {
            ...
            try {
                ...
                if (alive) setTrainers(response.items ?? []);
            } catch (error: unknown) {
                if (alive) setTrainersError(getErrorMessage(error, lang));
            } finally {
                if (alive) setTrainersLoading(false);
            }
        }
        void loadTrainers();
        return () => {
    ...
    const trainersById = React.useMemo(() => {
        ...
        for (const trainer of trainers) map.set(trainer.id, trainer);
        return map;
    }, [trainers]);
    ...
    async function handleDelete(user: AdminUser) {
        const confirmed = window.confirm(lang === "es" ? `¿Desactivar usuario "${user.name}"?` : `Deactivate user "${user.name}"?`);
        if (!confirmed) return;
    ...
    function handleOpenProfileImage(user: AdminUser) {
        ...
        if (mediaItem) setSelectedMedia(mediaItem);
    }
